...
def conditional_mapping(numbers):
    ...
    Returns:
        list: 매핑된 숫자 리스트
    """
    return [num**2 if num > 0 else 0 if num == 0 else abs(num) for num in numbers]
...
def flatten_with_condition(nested_list, threshold):
    ...
    """
    return [item for sublist in nested_list for item in sublist if item > threshold]
